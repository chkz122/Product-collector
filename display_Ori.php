...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $delete_sql = "DELETE FROM pro_Ori WHERE id=$delete_id";
        $conn->query($delete_sql);
    } elseif (isset($_POST['update_id'])) {
        $update_id = $_POST['update_id'];
        $name = $_POST['name'];
        $number = $_POST['number'];
        $update_sql = "UPDATE pro_Ori SET name='$name', number='$number' WHERE id=$update_id";
        $conn->query($update_sql);
    }
}

$sql = "SELECT id, name, number, image_path FROM pro_Ori";
...
<div id="popupForm" class="popup-form">
    ...
    <form id="changeForm" method="post" class="mini_form">
